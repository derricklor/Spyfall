        ...
        socket.on('message', (data) => {
            switch (data.type) {
                ...
                case 'voteCalled':
                    ...
                    break;
                case 'resetRoom':
                    setModalRevealRole(false);
                    setModalSpyGuess(false);
                    setView('waiting');
                    setRoomChat(prev => [...prev, data.message]);
                    setRole(null);
                    setLocation(null);
        ...
        switch(view) {
                            ...
                            <div className="col-span-1 lg:col-start-1 xl:col-start-2 space-y-6  mt-6">
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="flex items-center justify-center gap-2">
                                        ...
                                        py-2 px-2 rounded-lg font-medium transition duration-500 shadow-md">
                                            {isCodeCopied ?
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-5 gap-6 p-8 mt-4 lg:mx-auto">
                        <PlayerContext.Provider value={{roomCode, playerName, playerCode, role}}>
                            {modalRevealRole ? createPortal( <RevealRoleCard location={location} role={role} onContinue={() => setModalRevealRole(false)} />
                                , document.body ) : null}
                            {modalSpyGuess ? createPortal( <SpyGuessCard locationsArr={locationsArr} serverURL={URL} onSpyGuessLocation={spyGuessLocation} onClose={() => setModalSpyGuess(false)} />
                                , document.body ) : null}
            ...
            case 'vote':
                return (
                    <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-5 gap-6 p-8 mt-4 lg:mx-auto">
                        <PlayerContext.Provider value={{roomCode, playerName, playerCode, role}}>
                            {modalSpyGuess && createPortal( <SpyGuessCard locationsArr={locationsArr} onSpyGuessLocation={spyGuessLocation} onClose={() => setModalSpyGuess(false)} />
                                , document.body )}
                            <div className="col-span-1 lg:col-start-1 xl:col-start-2 space-y-6 mt-6">
                                <PlayerCard location={location} role={role} setModalSpyGuess={setModalSpyGuess} />
